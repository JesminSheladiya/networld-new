...
function UserProfile({ open, onClose, onProfileUpdate, onRelationAccepted }) {
    ...
    const startEdit = () => {
        form.setFieldsValue({ fullName: user.fullName, phone: user.phone, gender: user.gender, birthDate: toBirthDatePickerValue(user.birthDate) });
        setPreview(user.profilePicture || null);
        setNewImg(null);
        setEditing(true);
    };
    ...
    const infoRows = [
        ...
        ...(user.birthDate ? [{
            label: "Birth Date",
            value: formatBirthDateWithAge(user.birthDate),
            ...
            >
                ...
                {editing && (
                    <Form className="auth-form up-edit-form" form={form} layout="vertical" onFinish={save}>
                        ...
                        <Form.Item className="auth-field" name="birthDate"
